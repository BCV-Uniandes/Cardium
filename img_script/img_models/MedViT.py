...
def MedViT_small(pretrained=False, pretrained_cfg=None, **kwargs):
    model = MedViT(stem_chs=[64, 32, 64], depths=[3, 4, 10, 3], **kwargs)
    return model
...
def MedViT_optim(pretrained=False, pretrained_cfg=None, **kwargs):
    model = MedViT(stem_chs=[32, 16, 32], depths=[2, 2, 2, 2], path_dropout=0.1, **kwargs)
    return model